...
class ConnectorSource(base.DataSource):
    ...
    def ids_metadata(self):
        rep = self.consumer.descriptionRequest(self.provider_url + "/api/ids/data", self.representation)
        if not "intake_connector_metadata" in rep:
            raise ValueError('Representation doesn\'t contain intake metadata')
        metadata = json.loads(rep["intake_connector_metadata"])
